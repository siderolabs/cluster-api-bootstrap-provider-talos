...
func (r *TalosConfigReconciler) genConfigs(ctx context.Context, scope *TalosConfigScope) (*TalosConfigBundle, []string, error) {
	...
	if scope.Cluster.Spec.ClusterNetwork != nil && scope.Cluster.Spec.ClusterNetwork.Pods != nil && len(scope.Cluster.Spec.ClusterNetwork.Pods.CIDRBlocks) > 0 {
		podSubnets = scope.Cluster.Spec.ClusterNetwork.Pods.CIDRBlocks
	}

	if scope.Cluster.Spec.ClusterNetwork != nil && scope.Cluster.Spec.ClusterNetwork.Services != nil && len(scope.Cluster.Spec.ClusterNetwork.Services.CIDRBlocks) > 0 {
